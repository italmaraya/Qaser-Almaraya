...
.offer{display:grid;grid-template-columns:1fr 1fr;gap:8px 18px;background:#eefaf3;border:1px solid #cdebd9;border-radius:12px;padding:12px 16px}
.offer .it{display:flex;align-items:center;gap:8px;font-size:13px}
.offer svg{color:#1a7f47;flex:none}
...
export async function buildVisaPdfHtml({ cards, lang = 'ar', fmtPrice, pageUrl }) {
  ...
  if (single) {
    const c = first;
    // Only what we offer — services the visa doesn't include are simply left out.
    const inc = SERVICES.filter((s) => c[s[0]]).map((s) => (en ? s[2] : s[1]));
    if (!inc.length) inc.push(en ? 'Full follow-up of your application' : 'متابعة كاملة لطلبك');
    h += '<div class="avoid"><h2>' + (en ? 'What we do for you' : 'ماذا نقدّم لك') + '</h2><div class="offer">' +
      inc.map((x) => '<div class="it">' + ICON.check + esc(x) + '</div>').join('') + '</div></div>';
