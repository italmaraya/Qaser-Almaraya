...
export async function sendApplicationEmail({ recipients, application }) {
  ...
  // price. Providers need this to invoice/settle correctly, so it's included
  // whenever it's set on the card, regardless of currency.
  const currency = application.cost_currency === 'USD' ? '$' : 'د.ع';
  const adultCost = Number(application.adult_cost) || 0;
  const childCost = Number(application.child_cost) || 0;
  ...
  const costLines = (adultCost || childCost)
    ? `تكلفة البالغ: ${adultCost.toLocaleString()} ${currency}\nتكلفة الطفل: ${childCost.toLocaleString()} ${currency}\nالإجمالي (${adultCount} بالغ${childCount ? ' / ' + childCount + ' طفل' : ''}): ${totalCost.toLocaleString()} ${currency}\n`
    : '';
