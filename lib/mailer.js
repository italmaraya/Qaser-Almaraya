...
export async function sendPackageBookingNotice({ booking, pkg }) {
  ...
  const methodLabel = booking.payment_method === 'office' ? 'الدفع في المكتب' : (booking.payment_method || 'غير محدد');
...
${booking.payment_proof_url ? 'إشعار الدفع: ' + booking.payment_proof_url : 'لم يُرفَق إشعار دفع (الدفع في المكتب)'}
