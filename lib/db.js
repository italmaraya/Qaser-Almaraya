...
export async function ensureSchema() {
  ...
  `;
  // When true, the customer tracking page shows an upload box for this
  // status — e.g. an internal status named "missing passport/document" can
  // be flagged so the customer can upload the file directly from tracking,
  // instead of only being told to call/WhatsApp.
  await sql`ALTER TABLE internal_statuses ADD COLUMN IF NOT EXISTS allows_customer_upload BOOLEAN DEFAULT false`;
