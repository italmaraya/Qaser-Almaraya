    ...
    for (let i = 0; i < SEED_PACKAGES.length; i++) {
      ...
      await sql`
        INSERT INTO packages (
          ...
          departs_ar, departs_en, price, child_price, badge_ar, badge_en, prefs,
          includes_ar, includes_en, hotels, flights, days, sort_order
        ) VALUES (
          ${p.cat}, ${JSON.stringify(p.countries || [])}, ${p.destAr}, ${p.destEn}, ${p.titleAr}, ${p.titleEn},
          ${p.nightsAr}, ${p.nightsEn}, ${p.departsAr}, ${p.departsEn}, ${p.price}, ${p.childPrice},
          ${p.badgeAr || ''}, ${p.badgeEn || ''}, ${JSON.stringify(p.prefs || [])},
          ${JSON.stringify(p.includesAr || [])}, ${JSON.stringify(p.includesEn || [])},
          ${JSON.stringify(p.hotels || [])}, ${JSON.stringify(p.flights || [])}, ${JSON.stringify(p.days || [])}, ${i}
        )
      `;
    }
  }
