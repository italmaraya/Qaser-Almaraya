...
export async function ensureSchema() {
  ...
  await sql`ALTER TABLE countries ADD COLUMN IF NOT EXISTS pdf_banner_url TEXT DEFAULT ''`;
  await sql`ALTER TABLE countries ADD COLUMN IF NOT EXISTS active BOOLEAN DEFAULT true`;
  ...
  `;
  // When true, the customer tracking page shows an upload box for any
  // application currently sitting in this customer-facing status — e.g. the
  // "Action required" status can be flagged so the customer can upload a
  // missing passport/document straight from tracking, instead of only being
  // told to call/WhatsApp.
  await sql`ALTER TABLE customer_statuses ADD COLUMN IF NOT EXISTS allows_customer_upload BOOLEAN DEFAULT false`;
  ...
  // What the customer uploads in response to an "action required" status
  // (e.g. a missing passport scan) — sent from the public tracking page and
  // visible immediately in the admin/staff dashboard.
  await sql`ALTER TABLE visa_applications ADD COLUMN IF NOT EXISTS customer_upload_url TEXT DEFAULT ''`;
  await sql`ALTER TABLE visa_applications ADD COLUMN IF NOT EXISTS customer_upload_at TIMESTAMPTZ`;
  ...
  await sql`ALTER TABLE packages ADD COLUMN IF NOT EXISTS rating NUMERIC DEFAULT 4.8`;
  await sql`ALTER TABLE packages ADD COLUMN IF NOT EXISTS pdf_banner_url TEXT DEFAULT ''`;
  await sql`ALTER TABLE packages ADD COLUMN IF NOT EXISTS excludes_ar JSONB DEFAULT '[]'`;
  await sql`ALTER TABLE packages ADD COLUMN IF NOT EXISTS excludes_en JSONB DEFAULT '[]'`;
  // Scheduled publishing + countdown / seats left
  await sql`ALTER TABLE packages ADD COLUMN IF NOT EXISTS publish_at DATE`;
  await sql`ALTER TABLE packages ADD COLUMN IF NOT EXISTS departure_date DATE`;
  await sql`ALTER TABLE packages ADD COLUMN IF NOT EXISTS seats_left INTEGER`;
  // Category list changed to: family, honeymoon, music, sports, umrah (Hajj & Umrah).
  // Move packages saved under the old categories so none disappear from the filters.
  await sql`UPDATE packages SET cat = 'honeymoon' WHERE cat = 'couples'`;
  await sql`UPDATE packages SET cat = 'music' WHERE cat = 'event'`;
  await sql`UPDATE packages SET cat = 'family' WHERE cat IN ('group', 'beach', 'adventure')`;
  ...
  // Destination search suggestions shown in the packages page search box
  // ("Most searched cities" chips + a plain "Popular cities" list), managed
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS popular_destinations (
      ...
      kind TEXT NOT NULL DEFAULT 'popular',
      sort_order INTEGER DEFAULT 0,
      created_at TIMESTAMPTZ DEFAULT now()
    )
  `;
  const destCount = await sql`SELECT COUNT(*)::int AS n FROM popular_destinations`;
  if (destCount[0].n === 0) {
    await sql`
      INSERT INTO popular_destinations (name_ar, name_en, kind, sort_order) VALUES
      ('بغداد', 'Baghdad', 'most_searched', 0),
      ('الموصل', 'Mosul', 'most_searched', 1),
      ('البصرة', 'Basrah', 'most_searched', 2),
      ('أربيل', 'Erbil', 'most_searched', 3),
      ('النجف', 'Najaf', 'most_searched', 4),
      ('كربلاء', 'Karbala', 'most_searched', 5),
      ('الناصرية', 'Nasiriyah', 'popular', 0),
      ('إسطنبول', 'Istanbul', 'popular', 1),
      ('دبي', 'Dubai', 'popular', 2)
    `;
  }

  // Countries the packages page advertises as tour destinations, grouped by
  // region — the "All Tour Countries" block — managed from the admin dashboard.
  await sql`
  ...
  `;
  await sql`ALTER TABLE tour_countries ADD COLUMN IF NOT EXISTS lat NUMERIC`;
  await sql`ALTER TABLE tour_countries ADD COLUMN IF NOT EXISTS lng NUMERIC`;

  // Banner photo + deal badge shown above each region's country list on the
  // packages page (matched to tour_countries rows by region name).
  await sql`
    CREATE TABLE IF NOT EXISTS tour_regions (
      ...
      region_en TEXT NOT NULL DEFAULT '',
      image_url TEXT DEFAULT '',
      deal_text_ar TEXT DEFAULT '',
      deal_text_en TEXT DEFAULT '',
      sort_order INTEGER DEFAULT 0,
      created_at TIMESTAMPTZ DEFAULT now()
    )
  `;

  // Customer review / testimonial cards for the packages page — text + an
  // optional photo, managed from the admin dashboard.
  await sql`
  ...
  `;
  await sql`ALTER TABLE package_reviews ADD COLUMN IF NOT EXISTS photo_url TEXT DEFAULT ''`;

  // Regions used to group tour countries with a banner-style card
  // (image + "All Adventures" pill + optional deal badge), managed from the
  // admin dashboard.
  await sql`
    CREATE TABLE IF NOT EXISTS tour_regions (
      id SERIAL PRIMARY KEY,
