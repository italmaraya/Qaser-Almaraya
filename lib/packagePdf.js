...
.hero-txt{position:absolute;inset:auto 22px 18px 22px;z-index:2;color:#fff}
...
.amen svg,.inc svg{color:#049dc5;flex:none}
...
.inc{display:grid;grid-template-columns:1fr 1fr;gap:6px 18px}
.inc div{display:flex;align-items:center;gap:7px}
 ...
 */
export function buildPackageVoucherHtml(o) {
  const { pkg, hotel, flight, days = [], includes = [], adults = 1, children = 0, perAdult, perChild, grandTotal, fmtPrice, lang = 'ar', startDate } = o;
  const en = lang === 'en';
  ...
  if (days.length) cells.push([en ? 'Programme' : 'البرنامج', days.length + (en ? ' days' : ' أيام')]);
  ...
  // ── Included
  if (includes.length) {
    h += '<div class="avoid"><h2>' + (en ? "What's included" : 'تشمل الباقة') + '</h2><div class="inc">' +
      includes.map((x) => '<div>' + ICON.check + esc(x) + '</div>').join('') + '</div></div>';
  }
