...
export async function getLegacyVisaData() {
  try {
    ...
    const legacyCountries = countries.map((c) => ({
      code: c.flag_code ? c.flag_code : String(c.id),
      name: c.name_ar,
      en: c.name_en,
      region: c.region || '',
    }));

    const countryCodeById = new Map(countries.map((c) => [c.id, c.flag_code ? c.flag_code : String(c.id)]));
